    ...
    with open(input_file, 'r') as f_in:
        with open(output_file, 'w') as f_out:
            for line in f_in:
                ...
                output_line = '\t'.join(parts[:18] + [rs_qtl_str])
                #print(f"Output line: {output_line}") 
                f_out.write(output_line + '\n')

qtl_dict = {}
with open('/data/slurm/tmmap/qtldataset/qtl.tsv', 'r') as f:
    for line in f:
        parts = line.strip().split('\t')
        key1 = parts[0] 
        key2 = parts[1]  
...
with open("/data/slurm/tmmap/hichip.txt", "r") as list_file:
    for line in list_file:
        ...
        try:
            ...
            output_path = 'epqtl.txt'  # Define output_path here
            open(output_path, 'w').close()
            print(f"start processing {line}")
            update_epqtl(qtl_dict, 'eprank.txt', output_path)
